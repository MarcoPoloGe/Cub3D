...
void	ft_render_player_arrow(t_pos pos, double dir_angle, t_img *frame)
{
	...
	temp = ft_rotate_point(ft_coord(pos.y, pos.x), ft_coord(pos.y - (DIR_VECTOR_LEN / 2), pos.x), dir_angle);
	dir.x = temp.x;
	dir.y = temp.y;

	//ft_render_line_pos(pos, dir, COLOR_PLAYER_DIR, frame);

	temp = ft_rotate_point(ft_coord(pos.y, pos.x), ft_coord(pos.y - (DIR_VECTOR_LEN / 3), pos.x), dir_angle + 135);
	side.x = temp.x;
	side.y = temp.y;
	...
	temp = ft_rotate_point(ft_coord(pos.y, pos.x), ft_coord(side.y, side.x), 90);
	side.x = temp.x;
	side.y = temp.y;
