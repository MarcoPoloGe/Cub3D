...
void	ft_render_wall_line(t_pos pos, t_ray ray, t_impact *impact, t_data *data)
{
	int			i;
	double		step_y;
	double		wall_height;

	wall_height = WINDOW_HEIGHT / (impact->distance * cos(ft_degrees_to_radian(ray.angle)));
	...
	while (i < wall_height)
	{
		ft_put_pixel_frame(data->frame.game, pos.y + i, pos.x, ft_get_wall_color(i * step_y, ft_get_nb_from_percent(impact->wall_x, impact->wall->width), impact->wall));
		i++;
	}
}
...
void	ft_render_walls(t_camera camera, t_data *data)
{
	...
	while (i < WINDOW_WIDTH)
	{
		if (ray_list[i].impact != NULL)
			ft_render_wall_line(ft_pos(WINDOW_HEIGHT / 2, i), ray_list[i], ray_list[i].impact, data);
		i++;
	}
}
